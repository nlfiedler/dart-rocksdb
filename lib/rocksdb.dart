...
abstract class RocksError implements Exception {
  ...
  @override
  String toString() => 'RocksError: $_msg';
}
...
class RocksDB<K, V> extends NativeFieldWrapperClass2 {
  ...
  static RocksError _getError(dynamic reply) {
    if (reply == -1) {
      return const RocksClosedError._internal();
    }
    if (reply == -2) {
      return const RocksIOError._internal();
    }
    if (reply == -3) {
      return const RocksCorruptionError._internal();
    }
    if (reply == -4) {
      return const RocksInvalidArgumentError._internal();
    }
    return null;
  }
...
class RocksIterator<K, V> extends NativeFieldWrapperClass2
  ...
  RocksIterator._internal(RocksIterable<K, V> it)
...
/// An [Iterable<RocksItem>] for iterating over key-value pairs.
///
...
class RocksIterable<K, V> extends IterableBase<RocksItem<K, V>> {
  ...
  RocksIterable._internal(RocksDB<K, V> db, int limit, bool fillCache, K gt,
